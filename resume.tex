...
\newcommand{\cvItemT}[1]{\item[]{#1\vspace{-0.25em}}}
...
\newcommand{\cvItemGithubLink}[1]{\cvItemT{\ghlink{#1}}}
...
\begin{document}
	...
	\cvEntryNTPD{Казанский федеральный университет}{2023 -- present}{Фундаментальная информатика (магистратура)}{
		\cvItemSkillset{
	...
	\cvEntryNTPD{Казанский федеральный университет}{2019 -- 2023}{Фундаментальная информатика (бакалавриат)}{
		\cvItemSkillset{
			\item Вышмат
	...
	\cvEntryNTPD
	{Тинькофф}{январь 2023 -- июль 2023}
	{Стажёр-разработчик (департамент ИТ-инфраструктуры)}{
		\cvItem{
			Чат-бот, отслеживающий новые релизы на GitHub
		}
		\cvItem{
		...
		\cvItem{
			...
			Развитие системы генерации конфигурации prometheus
		}
	}
	...
	\section{Проекты}{\projectsIcon}
	\cvEntryWorkND{Чат-бот, отслеживающий новые релизы на GitHub}{
		\cvItemT{Бот выводит уведомления о новых версиях, прикрепляет changelog и при необходимости позволяет заводить задачки на обновление в Jira. Бот использует S3 для хранения состояния, развёрнут в k8s и конфигурируется через git.}
		\cvItemSkillset{
			\item Python
			\item Pydantic
			\item FastAPI
	...
	\cvEntryWorkND{Визуализация инфраструктуры}{
		\cvItemT{
			Делал панель для мониторинга серверов. Панель содержит довольно большое число правил, для автоматической генерации написан скрипт, полагающийся на два jinja-темплейта.
		}
		\cvItemSkillset{
			\item Python
			\item Poetry
	...
	\cvEntryWorkND{Развитие системы генерации конфигурации prometheus}{
		\cvItemT{
			На замену ранее существовавшему костыльному коду создан jinja-темплейт и разработан yaml-based DSL для описания устройств и автоматической генерации таргетов prometheus.
		}
		\cvItemSkillset{
			\item Python
			\item Poetry
	...
	\cvEntryPersonalND{Homelab}{
		\cvItemGithubLink{HectorHW/nginx\_configurator}
		\cvItemT{
			Набор сервисов, развёрнутых на моём домашнем сервере + Cloudflare, для которых я потихоньку делаю автоматизацию и мониторинг.
		}
		\cvItemSkillset{
			\item Linux
			\item Nginx
	...
	\cvEntryAcademicalND{Интерпретируемый язык программирования}{
		\cvItemGithubLink{HectorHW/blop-lang}
		\cvItemT{
			В рамках курсовой работы написал интерпретатор функционального языка программирования, включающий парсер, байткодовую виртуальную машину и сборщик мусора.
		}
		\cvItemSkillset{
			\item Rust
			\item PEG parsing
			\item Interpreter
			\item Language runtime
			\item GC
		}
	}

	\cvEntryAcademicalND{Грамматический фаззер}{
		\cvItemGithubLink{HectorHW/bocchifuzz}
		\cvItemT{
			В рамках дипломной работы разработал фаззер, позволяющий описывать входные данные в виде грамматики либо набора начальных образцов и проводить фазз-тестирование без модификации или перекомпиляции исследуемых файлов.
		}
		\cvItemSkillset{
			\item Rust
			\item PEG parsing
